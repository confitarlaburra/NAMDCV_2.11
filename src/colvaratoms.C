    ...
    noforce (false)
{
  ...
  for (cvm::atom_iter ai = this->begin();
       ai != this->end(); ai++) {
    total_mass += ai->mass;
     std::cout<<"total_massss "<<total_mass<<std::endl; 
  }
  total_charge = 0.0;
  for (cvm::atom_iter ai = this->begin();
       ai != this->end(); ai++) {
...
void cvm::atom_group::add_atom (cvm::atom const &a)
{
  if (b_dummy) {
    ...
    std::cout<<"total_massM "<<total_mass<<std::endl;
...
void cvm::atom_group::reset_mass(std::string &name, int i, int j)
{
  ...
  for (cvm::atom_iter ai = this->begin();
       ai != this->end(); ai++) {
    total_mass += ai->mass;
     std::cout<<"total_massR "<<total_mass<<std::endl;
  }
                             ...
                             char const        *key)
{
  ...
  if (get_keyval (group_conf, "dummyAtom", dummy_atom_pos, cvm::atom_pos(), mode)) {
    ...
    this->total_charge = 0.0;
     std::cout<<"total_massD "<<total_mass<<std::endl;
  } else
    b_dummy = false;
  ...
  if (!b_dummy) {
    ...
    for (cvm::atom_iter ai = this->begin();
      ...
      this->total_charge += ai->charge;
       std::cout<<"total_massNOd "<<total_mass<<std::endl;
    }
  }
...
cvm::atom_pos cvm::atom_group::center_of_mass() const
{
  ...
  for (cvm::atom_const_iter ai = this->begin();
       ai != this->end(); ai++) {
    com += ai->mass * ai->pos;
    //std::cout<<"ai->mass:"<<ai->mass<<" ai->pos:"<<ai->pos<<std::endl;
  }
  com /= this->total_mass;
  return com;
}
