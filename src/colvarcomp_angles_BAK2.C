...
void colvar::dipole_angle::calc_value()
{
  ...
  r21 = cvm::position_distance(g2_pos, group1.dipole(g1_pos)); //ORIGINAL!!
  //r21 = cvm::position_distance(g2_pos, g1_pos);
  r21l = r21.norm();
  r23  = cvm::position_distance (g2_pos, g3_pos);
  r23l = r23.norm();
...
void colvar::dipole_angle::calc_gradients()
{
  ...
  dxdr1 = (180.0/PI) *dxdcos *
    (1.0/r21l)* (r23/r23l + (-1.0)* cos_theta * r21 / r21l) ; //implementacion nueva
    //dxdr1 = (180.0/PI) * dxdcos *
  ...
  cvm::atom_pos total_force = (0,0,0);
  ...
  for (size_t i = 0; i < group1.size(); i++) {
    ...
    //group1.total_charge)* (dxdr1);// optimizado 1

    //group1[i].grad =(group1[i].charge + (-1)* group1[i].mass *
    //group1.total_charge / group1.total_mass) * (dxdr1);// optimizado 2, anda mejor

    group1[i].grad =(group1[i].mass/group1.total_mass) *(group1[i].charge + (-1)* group1[i].mass * aux1) * (dxdr1);// optimizado 3

    //group1[i].grad =(group1[i].charge + (-1)* group1[i].mass * aux1) * (dxdr1);// optimizado 3

    total_force +=  (group1[i].charge + (-1)* group1[i].mass * aux1) * (dxdr1);
    //group1[i].grad = (group1[i].mass/group1.total_mass) *  (dxdr1);
    //group1[i].grad =(group1[i].charge) * (dxdr1);//para el agua, anda mejor para el agua
    //group1[i].grad = 0 ;
    std::cout<<"grad1[i] "<<(group1[i].charge + (-1)* group1[i].mass * aux1) * (dxdr1) <<std::endl;
    std::cout<<"group1[i].charge "<<group1[i].charge<<std::endl;
    std::cout<<"group1[i].mass "<<group1[i].mass<<std::endl;
  ...
  for (size_t i = 0; i < group2.size(); i++) {
    group2[i].grad = (group2[i].charge + (-1)* group2[i].mass * aux2) *
      (dxdr1 + dxdr3) * (-1.0);
  }*/   

  for (size_t i = 0; i < group2.size(); i++) {
    group2[i].grad = (group2[i].mass/group2.total_mass)*(dxdr1 + dxdr3) * (-1.0);
    }

  for (size_t i = 0; i < group3.size(); i++) {
    group3[i].grad =(group3[i].mass/group3.total_mass) * (group3[i].charge + (-1)* group3[i].mass * aux3) *
      (dxdr3);
      }
