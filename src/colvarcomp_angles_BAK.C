...
void colvar::angle::calc_value()
{
  ...
  std::cout<< "colvar_value "<<x.real_value <<std::endl;
  std::cout<< "total_co2 "<<cos_theta * cos_theta <<std::endl;
}
...
void colvar::dipole_angle::calc_value()
{
  ...
  std::cout<< "calc_value"<<std::endl;
  group1.read_positions();
  group2.read_positions();
  group3.read_positions();

  cvm::atom_pos const g1_pos = group1.center_of_mass();
  std::cout<< "atom_pos g1 "<<group1.center_of_mass()<<std::endl;
  cvm::atom_pos const g2_pos = group2.center_of_mass();
  cvm::atom_pos const g3_pos = group3.center_of_mass();
  std::cout<< "atom_pos g2 "<<group2.center_of_mass()<<std::endl;
  std::cout<< "atom_pos g3 "<<group3.center_of_mass()<<std::endl;

  //r21 = group1.dipole(g1_pos);
  r21 = cvm::position_distance(g2_pos, group1.dipole(g1_pos));
  //r21 = cvm::position_distance(g2_pos, g1_pos);
  std::cout<<"dipole module "<<sqrt (r21[0]*r21[0]+r21[1]*r21[1]+r21[2]*r21[2])<<std::endl;
  r21l = r21.norm();
  std::cout<<"dipole norm "<<r21l <<std::endl;
  std::cout<<"dipole "<<r21 <<std::endl;
  r23  = cvm::position_distance (g2_pos, g3_pos);
  r23l = r23.norm();
...
{
  std::cout<< "calc_gradients"<<std::endl;
  cvm::real const cos_theta = (r21*r23)/(r21l*r23l);
  //en este caso x es la variable colectiva, y dxdcos es la derivada
  //de la variable colectiva respecto a cos \theta
  //b_1site_force = true;
  std::cout<< "b_1site_force "<<b_1site_force <<std::endl;
  cvm::real const dxdcos = -1.0 / std::sqrt (1.0 - cos_theta*cos_theta);

  dxdr1 = (180/PI)*dxdcos; //implementacion nueva
  //std::cout<< "180.0/PI "<<180.0/PI <<std::endl;
  std::cout<< "cos_theta "<< cos_theta <<std::endl;

  //dxdr1 = (180.0/PI) * dxdcos *
  //(1.0/r21l) * ( r23/r23l + (-1.0) * cos_theta * r21/r21l );//comentar para implementacion nueva

  double total_charge = 0.0;
  for (size_t i = 0; i < group1.size(); i++) {
    //std::cout<<"CHARGE "<<group1[i].charge<<std::endl;
    total_charge += group1[i].charge;
  }
  
  double total_force=0.0;
  double total_cos2=0.0;
  for (size_t i = 0; i < group1.size(); i++) {
    /*group1[i].grad = ((group1[i].charge - ((group1[i].mass/group1.total_mass) *
		total_charge) * (r23/r23l))/r21l - cos_theta*(r21/r21l)*(1.0/r21l)*(group1[i].charge -
						     ((group1[i].mass/group1.total_mass)
						     * total_charge) )) * (dxdr1);*/// primero

    /*group1[i].grad = (1/r21l)*((group1[i].charge - ((group1[i].mass/group1.total_mass) *
		total_charge) * (r23/r23l)) - cos_theta*(r21/r21l)*(group1[i].charge -
						     ((group1[i].mass/group1.total_mass)
						     * total_charge) )) * (dxdr1);*/// solo factorizado el modulo del dipolo, funciona igual a "primero"

    group1[i].grad =(group1[i].charge - (group1[i].mass/group1.total_mass) *
     total_charge)*  (1/r21l)* ((r23/r23l) - cos_theta*(r21/r21l)) * (dxdr1);// optimizado
    
    /*group1[i].grad =(group1[i].charge) *  (1/r21l)*( (r23/r23l) - cos_theta*(r21/r21l)) * (dxdr1);*///para el agua
    
    
    std::cout<<"group1[i].mass/group1.total_mass * total_charge "<<group1[i].mass/group1.total_mass * total_charge<<std::endl;
    std::cout<<"group1[i].mass/group1.total_mass "<<group1[i].mass/group1.total_mass<<std::endl;
    std::cout<<"total_charge "<<total_charge<<std::endl;
    std::cout<<"r23/r23l "<<r23/r23l <<std::endl;
    std::cout<<"group1["<<i<<"]"<<group1[i].grad <<std::endl;

    /*total_force +=  ((group1[i].charge - (group1[i].mass/group1.total_mass *
					   total_charge) * (r23/r23l))/r21l -
      cos_theta*(r21/r21l)*(1/r21l)*(group1[i].charge -(group1[i].mass/group1.total_mass *
      total_charge) )) * (dxdr1);*/ 

    total_force += (group1[i].charge - ((group1[i].mass/group1.total_mass) *
					  total_charge)/r21l) * ((r23/r23l) - cos_theta  * (r21/r21l)) * (dxdr1);

    //total_force_Ox = (180.0/PI)*dxdcos;
    total_cos2 = cos_theta *cos_theta ;
    
    //(group1[i].mass/group1.total_mass);
  }
  std::cout<<"TOTAL_COS2 "<<total_cos2<<std::endl;
  std::cout<<"TOTAL_FORCES "<<total_force<<std::endl;
  /*
  for (size_t i = 0; i < group1.size(); i++) {
    group1[i].grad = (group1[i].mass/group1.total_mass) *
      (dxdr1);
    std::cout<<"TOTAL FORCE "<<group1[i].grad<<std::endl;
    
    // total_force += (group1[i].mass/group1.total_mass) *
    //(dxdr1);
...
{
  std::cout<< "calc_force_invgrads"<<std::endl;
  // This uses a force measurement on groups 1 and 3 only
  // to keep in line with the implicit variable change used to
  // evaluate the Jacobian term (essentially polar coordinates
  // centered on group2, which means group2 is kept fixed
...
{
  std::cout<< "calc_Jacobian_derivate"<<std::endl;
  // det(J) = (2 pi) r^2 * sin(theta)
  // hence Jd = cot(theta)
  const cvm::real theta = x.real_value * PI / 180.0;
  jd = PI / 180.0 * (theta != 0.0 ? std::cos(theta) / std::sin(theta) : 0.0);
...
{
  std::cout<<"apply_force"<<std::endl;
  if (!group1.noforce){
    group1.apply_colvar_force (force.real_value);
    std::cout<<"force.real_value "<<force.real_value<<std::endl;
    std::cout<<"SALIDA DE APPLY FORCE AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"<<std::endl;
    //std::cout<< "group1.noforce = "<<group1.noforce<<std::endl;
  }
