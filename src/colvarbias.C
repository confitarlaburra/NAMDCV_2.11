...
{
  bias_energy = 0.0;
  //std::cout << b_chg_centers << b_chg_force_k << std::endl;
  if (cvm::debug())
    ...
    if (target_nstages) {
      ...
      if (cvm::step_absolute() % target_nsteps == 0 &&
        ...
        if (stage < target_nstages) {
          ...
          force_k = starting_force_k + (target_force_k - starting_force_k)
                    * std::pow (lambda, force_k_exp);
	  // posible_lugar calculo de fuerza
          cvm::log ("Harmonic restraint " + this->name + ", stage " +
              cvm::to_str(stage) + " : lambda = " + cvm::to_str(lambda));
          cvm::log ("Setting force constant to " + cvm::to_str (force_k));
        }
  ...
  for (size_t i = 0; i < colvars.size(); i++) {
    //std::cout<<"colvars.size "<<colvars.size()<<std::endl;
    //funcion calce, aca se calcula la fuerza que se va al carajo
    //Esta es la fuerza que muestra cuando uno coloca outputAppliedForce
    colvar_forces[i] =
      (-0.5) * force_k /
      (colvars[i]->width * colvars[i]->width) *
      colvars[i]->dist2_lgrad (colvars[i]->value(),
      colvar_centers[i]);
    /*std::cout<<"force_k "<<force_k<<std::endl;
    std::cout<<"colvars_forces[i] "<<colvar_forces[i]<<std::endl;
    std::cout<<"colvars[i]->value "<<colvars[i]->value() <<std::endl;
    std::cout<<"colvars[i]->width "<<colvars[i]->width<<std::endl;
    std::cout<<"colvars[i]->dist2_lgrad "<<colvars[i]->dist2_lgrad(colvars[i]->value(),  
    colvar_centers[i])<<std::endl;
    std::cout<<"colvar_centers[i] "<<colvar_centers[i]<<std::endl;
    std::cout<<"colvars[i] "<<colvars[i]<<std::endl;
    */
